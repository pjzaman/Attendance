...
Write-Host "  1. Unzip"
Write-Host "  2. Drop firebase-sa.json from Firebase Console next to apon-bridge.exe"
Write-Host "  3. Right-click install\install.ps1 → Run as Administrator"
Write-Host "  4. Answer two prompts (Bridge ID + SA path); installer does the rest."
Write-Host ""
Write-Host "Then HR opens the Flutter app → Settings → Integrations → Devices,"
Write-Host "edits the placeholder device record's IP/port/comm-key, and saves."
Write-Host "Bridge picks it up automatically within ~1 minute."
